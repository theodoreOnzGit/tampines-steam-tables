...
\section{Avoiding Shock Calculation Altogether}

The main purpose of shock calculation is to obtain the end state. That is,
what is the enthalpy and (especially) entropy of the steam leaving the nozzle.
The key calculation is to obtain an accurate velocity.
Perhaps, we could skip shock calculation altogether.

Considering that at choked flow, we will have a fixed flowrate $\dot{m}$.

\begin{equation*}
	\dot{m} = (\rho v A)_\text{outlet} 
\end{equation*}

We have a fixed mass flux or mass velocity. 

\begin{equation*}
	(\rho v)_\text{outlet} = \frac{\dot{m}}{A_\text{outlet}}
\end{equation*}

We also specify the pressure at the outlet, so that is known.
The stagnation enthalpy at the outlet is known too, since this is an 
adiabatic nozzle with no work done.

\begin{equation*}
	h_0 = h_\text{outlet} + \frac{v_\text{outlet}^2}{2}
\end{equation*}

We can skip shock calculation, and just iterate enthalpy straight using a 
(p,h) flash algorithm:

\begin{itemize}
	\item first, we guess $h_\text{outlet}$
	\item we calculate the $v_\text{outlet}$
	\item $(p,h)_\text{outlet}$, we obtain $\rho_\text{outlet}$
	\item with $\rho_\text{outlet}$ and $v_\text{outlet}$, we can calculate 
		velocity/flux $\rho v$
	\item compare this against the reference mass flux
	\item use bisection or some method to iterate until this is correct 
	\item constraint on the fact entropy MUST increase or be constant
\end{itemize}

Now this will let us know the end thermodynamic state and outlet velocity.
Don't even need to calculate shocks and such, which saves us a lot of 
trouble.

If we want to calculate shocks after this, it will be much easier.

How are shocks calculated then?

We now have the entropy before and after, as well as stagnation enthalpy 
before and after. Plus mass flowrates.

With this iteration procedure, we can just guess the area of the shock.
With this area, use (p,s) or (h,s) algorithms on both sides. This will 
allow us to obtain mass flux at both ides of the shock.

At the end of the day, we may avoid plotting Fanno and Rayleigh lines 
altogether, and use a (p,h) algorithm to iteratively obtain the outlet 
conditions instead.
