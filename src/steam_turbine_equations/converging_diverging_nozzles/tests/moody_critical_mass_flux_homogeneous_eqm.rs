...
/// Instead of validating G given stagnation conditions,
/// we fix throat conditions and recover stagnation conditions.
/// Then check if the recovered (p_0, h_0, G*) matches Moody's chart.
#[test]
#[ignore]
fn isobar_pref_1_00_inverse() {
    let p_ref = Pressure::new::<pound_force_per_square_inch>(100.0);
    let h_ref = AvailableEnergy::new::<btu_it_per_pound>(100.0);
    let g_ref: MassFlux = MassRate::new::<pound_per_second>(1000.0) 
                        / Area::new::<square_foot>(1.0);

    let dimensionless_stagnation_pressure = 1.00;
    let p0_expected = dimensionless_stagnation_pressure * p_ref;

    // scan throat pressures from p0 down to p_min
    // for each throat pressure, scan throat qualities from 0 to 1
    let p_min = Pressure::new::<megapascal>(0.000_611_212_677 * 1.01);
    let n_pressure_steps = 20;
    let n_quality_steps = 20;

    let dp = (p0_expected - p_min) / n_pressure_steps as f64;

    let mut results: Vec<(f64, f64, f64)> = vec![];
    // (h0/h_ref, g/g_ref, p0/p_ref)

    let mut p_t = p0_expected;

    for _ in 0..n_pressure_steps {
        p_t -= dp;
        ...
        let t_sat = sat_temp_4(p_t);
        let s_f = s_tp_1(t_sat, p_t);
        let s_g = s_tp_2(t_sat, p_t);

        for j in 0..=n_quality_steps {
            let x_t = j as f64 / n_quality_steps as f64;

            // interpolate entropy at throat quality
            let s_t = s_f + x_t * (s_g - s_f);

            // recover stagnation conditions from throat
            let (p_0, h_0, g_crit) = 
                get_stagnation_conditions_from_throat_ps(p_t, s_t);

            // dimensionless quantities
            let h0_dimless = h_0 / h_ref;
            let g_dimless = g_crit / g_ref;
            let p0_dimless = p_0 / p_ref;

            dbg!(&(h0_dimless, g_dimless, p0_dimless));

            results.push((
                h0_dimless.get::<ratio>(),
                g_dimless.get::<ratio>(),
                p0_dimless.get::<ratio>(),
            ));
        }
    }

    // filter results to only those near p0/p_ref = 1.00
    // tolerance of ±5% on stagnation pressure
    let p0_tolerance = 0.05;
    let filtered: Vec<(f64, f64)> = results.iter()
        .filter(|(_, _, p0_dimless)| {
            (p0_dimless - dimensionless_stagnation_pressure).abs() 
                < p0_tolerance * dimensionless_stagnation_pressure
        })
        .map(|(h0, g, _)| (*h0, *g))
        .collect();
    ...
    // for each Moody data point, find the closest filtered result
    // and check if G matches within tolerance
    for (h0_moody, g_moody) in data.iter() {
        let closest = filtered.iter()
            .min_by(|(h0_a, _), (h0_b, _)| {
                let da = (h0_a - h0_moody).abs();
                let db = (h0_b - h0_moody).abs();
                da.partial_cmp(&db).unwrap()
            });

        if let Some((h0_found, g_found)) = closest {
            dbg!(&(h0_moody, g_moody, h0_found, g_found));
            approx::assert_relative_eq!(
                *g_moody.log10(),
                g_found.log10(),
                max_relative = 1e-2
